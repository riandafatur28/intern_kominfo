...
import { LayoutDashboard, User, LogOut, X, ChevronLeft, ChevronRight, FileText } from 'lucide-react';

const navItems = [
    { to: '/team-lead/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
    { to: '/team-lead/permintaan-persetujuan', icon: FileText, label: 'Permintaan Persetujuan' }, 
    { to: '/team-lead/profil', icon: User, label: 'Profil Saya' },
];

export default function SidebarTeamLead({ collapsed, onToggle }) {
    ...
    return (
        <aside className={`bg-white border-r border-[#DDDEEE] flex flex-col h-screen fixed left-0 top-0 transition-all duration-300 z-20 ${
            ...
            {/* Footer Profile */}
            <div className={`p-3 border-t border-gray-100 mt-auto relative ${collapsed ? 'flex justify-center' : 'flex items-center justify-between'}`}>
                {collapsed ? (
